...
class Haiku extends Component {
  ...
  handleSelect = (input) => {
    ...
    const sylls = parseInt(
      input.target.options[input.target.selectedIndex].dataset.syll
    );
    //set state
    this.setState({
      userSelect: {
  ...
  render() {
    return (
      <div className="Haiku">
        ...
        <p>Line 1: {this.state.lineInProgress.toString()}</p>
        ...
        <form>
          ...
          <select name="wordSelect" id="word" onChange={this.handleSelect}>
        ...
        <div className="controls">
          ...
          <button className="removeEverything" onClick={this.removeEverything}>
            Remove everything
          </button>
        </div>
      </div>
